...
func (s *ServerSideRenderService) registerPacscriptSSRData() {
	AddTemplate(
		regexp.MustCompile(`^/packages/([a-zA-Z0-9-]+)`),
		func(path string, groups []string) IndexTemplateData {
			name := groups[1]

			pkg, err := array.FindBy(s.packageCacheService.GetAll(), func(s *pac.Script) bool {
			...
			return IndexTemplateData{
				...
				Html: fmt.Sprintf(`
					...
					pkg.PackageName,
					pkg.PackageName,
					consts.PACSCRIPT_FILE_EXTENSION,
					pkg.PackageName,
				),
			}
		},
