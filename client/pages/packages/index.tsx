...
const Search: FC<{ placeholder: string }> = ({ placeholder }) => {
    return (<div className="my-2 flex sm:flex-row flex-col">
        <div className="flex flex-row mb-1 sm:mb-0">
            {/* <div className="relative">
                <select
                    className="focus:outline-none appearance-none h-full rounded-l border block appearance-none w-full bg-white border-gray-400 text-gray-700 py-2 px-4 pr-8 leading-tight focus:outline-none focus:bg-white focus:border-gray-500">
                    <option>5</option>
                    <option>10</option>
                    <option>20</option>
                </select>
...
const PackageTable: FC<{ packages: PackageInfo[] }> = ({ packages }) => {
    ...
    const pickRandomPackage = () => packages[Math.floor(Math.random() * packages.length)].name
                ...
                <div className="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                    <div className="py-2 align-middle sm:px-6 lg:px-8 block" style={{ margin: 'auto' }}>
                        <div className="packages-wrapper overflow-hidden border-b border-gray-200 sm:rounded-lg">
                            <div>
                                <Search placeholder={randomPackage} />
                                <button
...
const Packages: NextPage = () => {
    ...
    useEffect(() => {
        ...
        axios.get<PackageInfo[]>(`${serverConfig.host}/api/packages?page=${page}`).then(pkgs => {
    ...
    return <>
        ...
        <div className="no-scrollbar">
            <div className="py-8 px-4 overflow-y-auto" style={{ marginLeft: 'auto', marginRight: 'auto', height: 'calc(100vh - 80px)' }} onScroll={() => onScroll()} ref={ref as any}>
                {loading ? <h1 className="text-center">Loading...</h1> : <PackageTable packages={packages} />}
            </div>
        </div>
    </>
}
