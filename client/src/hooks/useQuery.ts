...
const useQuery = <T>(
    ...
    useEffect(() => {
        const result = schema.validate(objParams, {
            cache: true,
            abortEarly: false,
            stripUnknown: true,
            convert: true,
        ...
        if (result.error) {
            navigate(
                location.pathname +
                    '?' +
                    new URLSearchParams(defaults as any).toString(),
                { replace: true },
            )
        } else {
    ...
    const setQuery = (query: T, replace?: boolean) => {
        const newParams = Object.entries(query).reduce(
        ...
        navigate(
            location.pathname + '?' + new URLSearchParams(newParams).toString(),
            { replace },
        )
